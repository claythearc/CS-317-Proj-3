...
def Kruskal(G : Graph):  # method to perform Kruskal's algorithm
	AdjMat = Adjacency()  # type: Adjacency
	for node in G:  # iterate the Graph in sorted order
		if not AdjMat.is_cycle(node, G):  # if the edge won't form a cycle
			AdjMat.addmst(node)  # add it to the minimum spanning tree
		elif AdjMat.is_final(node, G):
			AdjMat.addmst(node)
			break
